...
inicializa_matriz:
		push {r2-r9, lr}			@; guardar registros utilizados
		mov r4, r0					@; passem la direcció de la matriu a r4
		mov r3, r1					@; passem el mapa a r3
		mov r1, #0					@; inicialitzem "i (files)" en 0
		mov r6,  #ROWS				@; passem valor de ROWS a r6
		mov r7, #COLUMNS			@; passem valor de COLUMNS a r7
	.Lfori:
		mul r8, r1, r7				@; r8 = i * NC
		mov r2, #0					@; inicialitzem "j(columnes)" en 0
	.Lforj:
		add r9, r8, r2				@; r9 = i * NC + j
		@;ldrb r5, [r4, r9, lsl #2]
		@;cmp r5, #0xff
		@;beq .Lif1
	@;.Lif1:
		...
		add r2, #1					@; r2 = j + 1
		cmp r2, #ROWS				@; comparem j amb ROWS
		blo .Lforj					@; si j < ROWS saltem al bucle Lforj
		
		add r1, #1					@; r1 = i + 1
		cmp r1, #COLUMNS			@; comparem i amb COLUMNS
		blo  .Lfori					@; si i < COLUMNS saltem al bucle Lfori
				
		pop {r2-r9, pc}				@;recuperar registros y volver
